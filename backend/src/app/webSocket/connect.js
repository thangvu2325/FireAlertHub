...
async function sendEmailandSMStoUserifhasWarning(nodeWarning, user, station) {
  if (canSend) {
    const objEmail = {
    ...
    user.inbox.push({
      check: false,
      message: `Cảnh báo phát hiện cháy tại "${nodeWarning.map(
        (node) => `${node.node_name}`
      )}"
     vào lúc ${new Date().toLocaleString("pt-BR")}`,
    });
    user.save();
    // Send inbox to Tram chua chay
    User.find()
      .populate("roles", "-__v")
      .populate("usersManager", "-__v")
      .then((users) => {
        const userSelect = users.filter((e) => e.roles[0].name === station)[0];
        if (userSelect) {
          userSelect.inbox?.push({
            check: false,
            message: `Cảnh báo phát hiện cháy tại "${nodeWarning.map(
              (node) => `${node.node_name}`
            )}" ở vị trí ${user.location}
           vào lúc ${new Date().toLocaleString("pt-BR")}`,
          });
          userSelect.save();
        }
...
function broadcastMessagetoUser(
  ...
  if (!senderUserId) {
    // console.log(station, nodeList, gateway);
    User.find()
      .populate("roles", "-__v")
      .populate("usersManager", "-__v")
      .then((users) => {
    ...
    User.findById(senderUserId)
      ...
      .then((user) => {
        if (!user) {
          ...
          UserManager.findOne({
            Users: { $elemMatch: { gateway: gateway } },
          })
            .then((foundUser) => {
              if (foundUser) {
                const userSelect = foundUser.Users.find(
                  (e) => e.gateway === gateway
                );
                if (userSelect) {
                  userSelect.location = location;
                  foundUser
                    .save()
                    .then(() => {
                    ...
                    .catch((saveErr) => {
                      console.log("Error while saving:", saveErr);
                    });
                } else {
                  console.log("User with specified gateway not found.");
                }
              } else {
                console.log("UserManager with matching gateway not found.");
              }
            })
            .catch((err) => {
              console.log("Query error:", err);
            });
          handleIfhasWarning(user, nodeList, station);
          nodeList.forEach((e) => {
